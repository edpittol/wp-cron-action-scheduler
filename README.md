...
- **Action Scheduler: 4.0.0.** Pinned as the single source of truth in [`docker/composer.lock`](docker/composer.lock), resolved from `"wpackagist-plugin/action-scheduler": "4.0.0"` in [`docker/composer.json`](docker/composer.json) and installed by the one `composer install` step in `docker/Dockerfile` — the same mechanism that pins WordPress core (see issue #29 and [ADR-0001](docs/adr/0001-nginx-php-fpm-replaces-the-built-in-server.md)). There is no longer a `docker/Dockerfile` build arg for this; the earlier `ARG ACTION_SCHEDULER_VERSION` was retired when the webroot moved to Composer, and the Dockerfile's own opening comment records that hand-off. The pinned version is independently confirmed by reading `ActionScheduler_Versions::instance()->latest_version()` back from a live request — see the `action_scheduler_version` field in both raw JSON responses in [`docs/measurements/unhook-timing-4.0.0.md`](docs/measurements/unhook-timing-4.0.0.md) and in [`docs/measurements/full-scenario-matrix-report.md`](docs/measurements/full-scenario-matrix-report.md#unhook-timing-table-issue-8). Every preflight snapshot now also carries `action_scheduler_version_lockfile` — the lockfile's own resolved version — alongside that live reading, so a reader can confirm the two agreed at capture time from the record alone; see any record under [`results/matrix/20260802T152248Z/`](results/matrix/20260802T152248Z). This is enforced, not just reported: `wpcas_preflight_evaluate()` in [`docker/wp-cli/lib/preflight-assertions.php`](docker/wp-cli/lib/preflight-assertions.php) fails the preflight outright when the live version and the lockfile's disagree, which is how a stale webroot volume is caught rather than silently measured (issue #31, [ADR-0002](docs/adr/0002-shared-webroot-volume-staleness-detected-not-prevented.md)). All sources agree; there is no discrepancy to resolve.
