...
- **WordPress: 7.0.2, pinned via `docker/composer.lock` and recorded in every committed result.** The webroot moved from an unpinned `wp core download` to a Composer install (issue #29, [`docs/adr/0001`](docs/adr/0001-nginx-php-fpm-replaces-the-built-in-server.md#layout-core-under-wp-provisioned-by-composer-deliberately-not-bedrock)): `docker/composer.json` requires `johnpbloch/wordpress-core` at `7.0.2`, and `docker/composer.lock` is the single committed source of truth `composer install` resolves against at image build time. Every preflight snapshot — and therefore every result record, since every `measure*` subcommand runs preflight internally — carries both `wp_version` (read back from the live site) and `wp_version_lockfile` (read back from the lockfile), so a mismatch between the two fails preflight loudly instead of silently measuring stale software (see the shared-webroot-staleness caveat below). Both fields read `7.0.2` in agreement across every scenario of the committed run at `results/matrix/20260802T152248Z/` — see, for example, the `preflight` object in [`results/matrix/20260802T152248Z/20260802T152250Z-wp-cron.json`](results/matrix/20260802T152248Z/20260802T152250Z-wp-cron.json).

## Caveats

- **The shared webroot volume goes stale silently unless preflight catches it.** `docker-compose.yml`'s `webroot` named volume is populated once, by Composer, on the container's first start (`docker/Dockerfile`'s Composer step) — a later image rebuild, e.g. after bumping `docker/composer.lock`, does not reach an already-populated volume; the volume has to be removed before the next `bin/stack up` for the new lockfile to take effect. This stack does not prevent that gap; it detects it: `bin/stack preflight` reads the live WordPress and Action Scheduler versions back from the running site and refuses to measure when they disagree with what `docker/composer.lock` resolved, so the failure mode is a loud, non-zero preflight refusal naming the mismatch, not a plausible-looking result describing software nobody intended to measure. See [`docs/adr/0002-shared-webroot-volume-staleness-detected-not-prevented.md`](docs/adr/0002-shared-webroot-volume-staleness-detected-not-prevented.md).
- **WordPress core is Composer-managed, under `/wp`, not a plain `wp core download` at the document root.** `docker/composer.json` pins `johnpbloch/wordpress-core` (currently `7.0.2`) alongside Action Scheduler and the SQLite integration plugin, all resolved via `docker/composer.lock`; core installs to `/wp`, with `wp-content` nested beneath it in the ordinary place, not renamed or relocated the way a Bedrock-style layout would. This is deliberately not Bedrock — the only thing worth reproducing from the original environment is core's `/wp` path prefix, so this repo's entry-point paths and canary log lines can match a reader's own, not a full alternate directory convention. See [`docs/adr/0001`](docs/adr/0001-nginx-php-fpm-replaces-the-built-in-server.md#layout-core-under-wp-provisioned-by-composer-deliberately-not-bedrock).
