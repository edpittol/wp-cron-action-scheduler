...
- `occupancy [count]` seeds `count` due-now actions, triggers Action Scheduler's async dispatch via an admin-context HTTP hit, and fires waves of concurrent front-end requests at increasing concurrency (bracketing the PHP built-in server's worker pool: below, at, and twice above `PHP_CLI_SERVER_WORKERS`) while polling the drain's pending count out-of-band via WP-CLI, never over HTTP. It prints one finished JSON record naming the server model explicitly (`php-cli-server`), so the figures can never be misread as PHP-FPM.
...
- Rendered report: [`docs/measurements/full-scenario-matrix-report.md`](docs/measurements/full-scenario-matrix-report.md) — generated by `bin/generate-report` from the committed run at `results/matrix/20260729T174806Z/`.
...
This stack differs from the environment the original findings (`handoff-wp-cron-as-post-and-poc.md`) were measured against in three ways. Each is a real difference in how the code executes, not just cosmetic — read this section before treating a result here as a stand-in for the original environment's behaviour.

1. **PHP's built-in server, not PHP-FPM: there is no `fastcgi_finish_request()` call to mask the guard's response.** Core's `wp-cron.php` calls `fastcgi_finish_request()` before it loads mu-plugins, which under PHP-FPM flushes and closes the response to the client as an already-sent 200 before any guard code runs — the client sees 200 even though the guard's `exit` genuinely stopped the queue from draining. PHP's built-in server (`php -S`, used by `docker/entrypoint.sh` here) has no such call, so guard section 1's `http_response_code(403); exit;` reaches the client as a real, observable 403 (see the `http-cron-armed` row in [`docs/measurements/full-scenario-matrix-report.md`](docs/measurements/full-scenario-matrix-report.md)). This does **not** invalidate the guard's own logic — the `exit` still runs before any cron code either way, on both server models — but it **does** mean a reader cannot use this stack's HTTP status codes as a stand-in for what a PHP-FPM deployment's client would see; on PHP-FPM, a 200 is not evidence the guard failed, and this stack cannot demonstrate that specific, counter-intuitive gap (see the next section).
2. **The SAPI string is the built-in server's (`cli-server`), not PHP-FPM's (`fpm-fcgi`).** Guard sections 1 (`10-block-http-cron.php`) and 3 (`30-log-non-cli-canary.php`) both branch on `'cli' !== PHP_SAPI` / `'cli' === PHP_SAPI`, which is true for `cli-server` exactly as it would be for `fpm-fcgi` — the guards' actual blocking/detection behaviour is unaffected. (Section 4's unhook guard, `40-unhook-queue-runner.php`, exempts CLI a different way entirely — `if ( defined('WP_CLI') && WP_CLI ) { return; }` — so it doesn't touch `PHP_SAPI` at all.) What does change is anything that logs or displays `PHP_SAPI` verbatim: section 3's canary is the one that actually logs it, in `sprintf('[cron-guard] queue run outside CLI: sapi=%s uri=%s ip=%s', PHP_SAPI, ...)`, and this stack's probe records likewise show `sapi=cli-server` (see the `probe_records` field in any committed scenario record, e.g. `results/matrix/20260729T174806Z/20260729T174906Z-http-cron-unarmed.json`), where a PHP-FPM deployment's canary and logs would read `sapi=fpm-fcgi` instead. This does **not** invalidate the canary's detection logic, since the check is on the SAPI's *identity* (`cli` vs. not), not its literal string — but it **does** mean a reader searching production logs for `fpm-fcgi` to correlate with this repo's example output will not find a match; the string itself is not portable, only the CLI/non-CLI distinction is.
3. **The database is SQLite, not MySQL.** `docker/Dockerfile` installs the SQLite Database Integration plugin's `db.php` drop-in (with `WP_SQLITE_AST_DRIVER` enabled) instead of connecting to MySQL/MariaDB. Action Scheduler's claim query is an `UPDATE ... JOIN (subquery) ... SET ...`, which needs the plugin's newer, grammar-based AST translator to run on SQLite at all (its legacy regex-based translator cannot rewrite that shape of query). Every scenario in this repo that shows a successful drain is therefore also, incidentally, evidence that the AST translator handles Action Scheduler's claim query correctly — but it does **not** invalidate any of the guard/entry-point findings themselves, none of which depend on the specific SQL Action Scheduler issues or on MySQL-specific behaviour. It **does** mean this stack carries a residual risk the original MySQL environment did not: a future "zero drained" result on this stack should first rule out an AST-driver translation gap before being read as a guard finding, exactly the way this repo's own Dockerfile comments already flag.

## Findings this stack cannot demonstrate

Two of the original findings in `handoff-wp-cron-as-post-and-poc.md` are out of scope for this stack. Their absence from any result record here is not evidence they were disproven — it means the scenario was never run.

1. **"You cannot return an HTTP status from a mu-plugin on `wp-cron.php`"** (the original write-up's centrepiece finding, `handoff-wp-cron-as-post-and-poc.md` §3.A and post-outline section 5) — that a PHP-FPM client sees an already-flushed 200 while the guard genuinely blocked the queue. This is not merely unmeasured here; it is **structurally impossible to reproduce on this stack**, because it depends on `fastcgi_finish_request()`, which only exists under PHP-FPM. The handoff document says this explicitly: "PHP-FPM is mandatory. `fastcgi_finish_request()` does not exist under the PHP built-in server or CLI, so scenario A **cannot** be reproduced with `wp server` or a light/SQLite setup" (§5). This is the same fact as deviation 1 above, from the other direction: deviation 1 describes what this stack shows instead (a real 403); this entry states plainly that the original, response-masking finding itself has no scenario in this repo and cannot get one without swapping the server model entirely.
2. **"A cron run is serial and fragile — one uncaught exception stalls every event behind it"** (`handoff-wp-cron-as-post-and-poc.md` §3.B and post-outline section 6), including the associated methodology trap that an unrelated fatal error earlier in a run can produce a false "0 drained" result that looks like a passing block. Unlike finding 1 above, nothing about this stack's server model or database makes this **technically** unreproducible — the handoff document itself notes a MinIO/S3 dependency is not required, only "a deliberate `add_action('init', ...)`-scheduled throwing event" (§5). But no scenario in `bin/measure-matrix`, and no mu-plugin under `docker/mu-plugins-available/`, deliberately introduces a fatal error mid-drain to demonstrate this. It was out of scope of the issues this repo implements (#1–#10); a reader should not read the absence of a "fragile serial run" scenario here as evidence that this stack's own 50/50 drains would necessarily survive an uncaught exception partway through — that specific claim was never tested against this codebase.
