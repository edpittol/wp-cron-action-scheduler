...
- **Action Scheduler's own async loopback**, a POST to `admin-ajax.php?action=as_async_request_queue_runner`. Unarmed, this drains 50/50 in 17.971 seconds, HTTP 200 (`bin/stack measure-async-ajax async-ajax-unarmed`; `docs/measurements/full-scenario-matrix-report.md`, `async-ajax:async-ajax-unarmed` row).
- **The same POST, with guard sections 1–3 armed** — `wp-cron.php` blocked, async dispatch's *outbound* send suppressed, and a non-CLI canary logging — still drains 50/50 in 17.931 seconds, HTTP 200 (`bin/stack measure-async-ajax async-ajax-sections-1-2-3-armed`; `docs/measurements/full-scenario-matrix-report.md`, `async-ajax:async-ajax-sections-1-2-3-armed` row; the raw record's `outcome.drained` is 50, `outcome.fully_drained` is `true`). Suppressing *dispatch* only stops Action Scheduler from *sending* that request itself; it does not gate the endpoint against someone sending an equivalent request directly.
- **A manual "Run" click** on the Scheduled Actions admin screen, with *all four* guard sections armed — including the unhook. This still drains one pending action, HTTP 200, in 0.295 seconds (`bin/stack measure-manual-run manual-run-sections-1-2-3-4-armed`; `results/matrix/20260802T152248Z/20260802T152855Z-manual-run-manual-run-sections-1-2-3-4-armed.json`, `outcome.drained: 1`).
...
- `wp cron event run --due-now` fires through the same `WP_CRON_HOOK` that `wp-cron.php` uses. Its record shows `execution_contexts.started` as `{"WP Cron": 50}`, 50/50 drained in 12.365 seconds (`bin/stack measure wp-cron`; `results/matrix/20260802T152248Z/20260802T152250Z-wp-cron.json`).
- `wp action-scheduler run` uses `ActionScheduler_WPCLI_QueueRunner` and never touches `WP_CRON_HOOK` at all. Its record shows `execution_contexts.started` as `{"WP CLI": 50}` instead, 50/50 drained in 12.373 seconds (`bin/stack measure action-scheduler`; `results/matrix/20260802T152248Z/20260802T152313Z-action-scheduler.json`).
...
With `DISABLE_WP_CRON` true and section 1 armed, `wp-cron.php` returns a client-observable HTTP 200 and drains 0 of 50 (`bin/stack measure-http http-cron-armed`; `docs/measurements/full-scenario-matrix-report.md`, `http-cron-armed` row) — the masked response the project [`README`](../README.md#the-response-masking-finding-issue-35) documents in full: core's own `fastcgi_finish_request()` call flushes and closes the response with whatever status was current before section 1's mu-plugin ever runs, so the guard's real block only shows up in its own post-flush self-report (`post_flush_status: 403`), never in anything the client, nginx's access log, or PHP-FPM's own access log can read back — which is exactly why §8 below treats `drained: 0`, not the status code, as the load-bearing figure. At that point the web path was already contributing nothing — closing it doesn't remove a safety net that was catching cron runs, it retires a path that was already dead weight once `DISABLE_WP_CRON` had been set.

The other side of that same condition is the actual fallback case, and it's handled automatically rather than needing to be remembered: flip `DISABLE_WP_CRON` back to `false` with the same guard section still armed, and `wp-cron.php` drains 50/50 again, in 0.004 seconds, HTTP 200 (`bin/stack set-disable-wp-cron false` then `bin/stack measure-http http-cron-armed-cron-enabled`; `docs/measurements/full-scenario-matrix-report.md`, `http-cron-armed-cron-enabled` row). A site that still genuinely relies on the web loopback keeps it; a site that has already turned it off loses nothing by having the door locked as well as unused.

## 6. Capacity, measured on this stack's server — nginx + PHP-FPM

This project's stack runs nginx in front of PHP-FPM, with the pool pinned static at 6 workers (`pm = static`, `pm.max_children = 6`, written to `/usr/local/etc/php-fpm.d/zz-wpcas-pool.conf` by `docker/Dockerfile`). Every figure in this section describes that pool specifically.
...
- Server model: `nginx + php-fpm, 6 workers`. Workers occupied by the drain: **1** (`results/matrix/20260802T152248Z/occupancy-drain.json`, `workers_occupied_by_drain`). That figure is explicitly not a direct counter read — the record's own `workers_occupied_by_drain_basis` field states it's an architectural inference from PHP-FPM's one-request-per-child model, confirmed for this run by the continuous, gap-free pending-count decline actually observed, not a server-reported occupancy metric (no pool-status query was made for this run).
- Drain duration: **10.376 seconds**, `drain_completed: true`, `drain_observed_in_flight: true` (same file, `drain_duration_seconds`).
- Front-end latency during that drain, against a same-shape baseline measured before the drain started (`docs/measurements/full-scenario-matrix-report.md`, "Worker occupancy during a drain" table):
  ...
  |---|---|---|---|
  | 3 | 0.0436 | 0.0313 | -28.2% |
  | 6 | 0.0557 | 0.0454 | -18.5% |
  | 12 | 0.0732 | 0.0777 | +6.2% |
  | 24 | 0.1231 | 0.1332 | +8.2% |

  `degradation_observed` for this run is `false` — no concurrency level crossed the record's own >25% threshold. Two of the four deltas are negative (front-end requests measured faster during the drain at the lowest concurrencies) and two are positive but still well under the threshold; either way, this is noise from a 6-worker pool handling one extra concurrent request, not a real degradation.

On this server, one 50-action drain occupying one of six PHP-FPM pool workers for roughly 10 seconds did not produce a measurable front-end latency cost at any tested concurrency. That is a statement about this specific 6-worker static PHP-FPM pool with this workload's shape — a real PHP-FPM capacity figure now, not an inference carried over from a different server model.
