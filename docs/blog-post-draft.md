...
With `DISABLE_WP_CRON` true and section 1 armed, `wp-cron.php` returns 403 in 0.005 seconds and drains 0 of 50 (`bin/stack measure-http http-cron-armed`; `docs/measurements/full-scenario-matrix-report.md`, `http-cron-armed` row). At that point the web path was already contributing nothing — closing it doesn't remove a safety net that was catching cron runs, it retires a path that was already dead weight once `DISABLE_WP_CRON` had been set.
